...
sprite_size(struct sprite_pack *pack, int id) {
	if (id < 0 || id >=	pack->n)
		return 0;
	int8_t * type_array = (int8_t*)OFFSET_TO_POINTER(pack, pack->type);
...
sprite_action(struct sprite *s, const char * action) {
	...
	struct pack_action *pa = (struct pack_action *)OFFSET_TO_POINTER(s->pack, ani->action);
	if (action == NULL) {
		if (ani->action == 0) {
			return -1;
		}
...
sprite_init(struct sprite * s, struct sprite_pack * pack, int id, int sz) {
	...
	s->name = NULL;
	s->id = id;
	uint8_t *type_array = (uint8_t *)OFFSET_TO_POINTER(pack, pack->type);
...
sprite_trans_mul2(struct sprite_pack *pack, struct sprite_trans_data *a, struct sprite_trans *b, struct sprite_trans *t, struct matrix *tmp_matrix) {
	...
	t->mat = (struct matrix*)OFFSET_TO_POINTER(pack, a->mat);
	t->color = a->color;
...
draw_child(struct sprite *s, struct srt *srt, struct sprite_trans * ts, struct material * material) {
	...
	struct pack_animation *ani = s->s.ani;
	struct pack_frame * pf = (struct pack_frame *)OFFSET_TO_POINTER(s->pack, ani->frame);
	pf = &pf[frame];
	int i;
	int scissor = 0;
	for (i=0;i<pf->n;i++) {
		struct pack_part *pp = (struct pack_part *)OFFSET_TO_POINTER(s->pack, pf->part);
		pp = &pp[i];
...
sprite_matrix(struct sprite * self, struct matrix *mat) {
	...
	if (parent) {
		...
		struct pack_frame * pf = (struct pack_frame *)OFFSET_TO_POINTER(parent->pack, ani->frame);
		pf = &pf[frame];
		int i;
		for (i=0;i<pf->n;i++) {
			struct pack_part *pp = (struct pack_part *)OFFSET_TO_POINTER(parent->pack, pf->part);
			pp = &pp[i];
			int index = pp->component_id;
			struct sprite * child = parent->data.children[index];
			if (child == self) {
				child_mat = (struct matrix*)OFFSET_TO_POINTER(parent->pack, pp->t.mat);
				break;
			}
		}
...
child_aabb(struct sprite *s, struct srt *srt, struct matrix * mat, int aabb[4]) {
	...
	for (i=0;i<pf->n;i++) {
		...
		struct matrix temp2;
		struct matrix *ct = mat_mul((struct matrix *)OFFSET_TO_POINTER(s->pack, pp->t.mat), t, &temp2);
